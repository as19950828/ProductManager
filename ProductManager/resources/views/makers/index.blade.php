<x-app-layout>
    <x-slot name="header">
        <div class="flex gap-4">
            ...
            <form method="get" action="{{ route('makers.create') }}">
                <button
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form class="flex justify-end gap-4 my-4" method="get" action="{{ route('makers.index') }}">
                        <input type="text" name="search" id="search" placeholder="メーカー名で検索"
                        ...
                        <table class="table-auto w-full text-left whitespace-no-wrap text-center">
                            ...
                            <tbody>
                                @foreach ($makers as $maker)
                                    <tr>
                                        <td class="border-t-2 border-gray-200 px-4 py-3"> {{ $maker->id }}</td>
                                        <td class="border-t-2 border-gray-200 px-4 py-3">{{ $maker->name }}</td>
                                        <td class="border-t-2 border-gray-200 px-4 py-3">
                                            <div class="flex gap-2 justify-center">
                                                <form method="get"
                                                    action="{{ route('makers.show', ['id' => $maker->id]) }}">
                                                    <button
